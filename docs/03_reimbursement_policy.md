# Reimbursement Processing Policy v1.1
...
The baseline profile permits `venue`, `transport`, `printing`, `supplies`, `communication`, `approved_food`, and `approved_service`; treats `gift`, `honorarium`, `equipment`, and `late_submission` as conditional; and treats `alcohol`, `tobacco`, `illegal_goods`, and `personal_expense` as prohibited. Categories, thresholds, roles, and retention settings are configuration inputs requiring confirmation before real operation.
...
| 30 | `RULE-DUP-001` | A duplicate match is suspected, incomplete, or disputed. | `ESCALATED / FACT_UNKNOWN`. |
...
| 41 | `RULE-SCOPE-002` | An expense appears illegal or prohibited by applicable law. | `ESCALATED / OUT_OF_POLICY`; no internal exception is permitted. |
| 42 | `RULE-CAT-001` | A prohibited but not illegal category applies. | `ESCALATED / OUT_OF_POLICY`. |
| 43 | `RULE-CAT-002` | The category is alcohol. | `ESCALATED / OUT_OF_POLICY`; require the configured Club Chair and Parent Advisor review. |
...
The audit history is append-only. It records at least `RECEIVED`, `VALIDATED`, `RULE_TRIGGERED`, `CLASSIFIED`, `PAUSED`, `RESUMED`, `OVERRIDDEN`, and `UNDONE` events, each with the policy version, input hash, actor, time, triggered rules, and explanation needed to reproduce the result.
...
The canonical synthetic test set is [test_cases.json](../policy-forge-baseline/test_cases.json); the quick Verify suite is [verify_cases.json](../policy-forge-baseline/verify_cases.json). The suites cover both flows, routine boundaries, unreadable evidence, missing payment proof, inconsistent totals, prohibited categories, authority boundaries, budget, conflicts, and confirmed duplicate payment.
